...
<div id="content">
<div class="search">
<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>?page=7" method="post">
...
$result=$conn->query("SELECT * FROM keyuplog ".$condition." order by uid DESC LIMIT $start, $limit");

//$info=array("playerid","name","cash","bankacc","adminlevel","donatorlvl","coplevel","civ_talents","lastupdated");
$info=array("uid","playerid","playername","action","actionid","instanceid","time");
...
$result=$conn->query("select count(*) as num from keyuplog ".$condition);
$row=$result->fetch_assoc();
$rows=$row["num"];
$total=ceil($rows/$limit);
$url="?page=7&keyword=".$keyword;
