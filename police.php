...
<div id="content">
<div class="search">
<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>?page=3" method="post" >
...
while(($row = $result->fetch_assoc()))
{
	...
	{
		if($t=="playerid"){
			echo "<td><a href='?page=15&uid=".$row["uid"]."'>".$row[$t]."</a></td>";
		}else{
			echo "<td>".$row[$t]."</td>";
		}			
	}
...
$result=$conn->query("select count(*) as num from players as p, players_global as g, whitelist as w WHERE p.playerid=g.playerid and w.playerid=p.playerid and w.side='cop' ".$condition);
$row=$result->fetch_assoc();
$rows=$row["num"];
$total=ceil($rows/$limit);
$url="?page=3&keyword=".$keyword;
